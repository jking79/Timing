...
void wc_ku_InterCali_aveRecHit_mini( string indir, string infilelist, string outfilename ){
    ...
    const int  nAlgos = 7; // RtStc, RtOOTStc, WtStc, WtOOTStc, MfootStc, MfootCCStc, pCalo
    //const int  nPhotons = 4;
    const double offset = 0.0;
    const int bin_offset = 86;
    const float kscc_offset = 3351.0;
...
//    auto fInTree = (TTree*)fInFile->Get(disphotreename.c_str());
         ...
         string algostring[nAlgos] = { "RtStc", "RtOOTStc", "WtStc", "WtOOTStc", "MfootStc", 
														"MfootCCStc", "pCalo" };
    ...
    std::map<UInt_t,Float_t> sumXtalpCaloRecTime;
         ...
         for (Long64_t centry = 0; centry < nEntries; centry++){
           ...
           //if( centry > 1000 ) break;
             ...
             for (auto i = 0U; i < nRecHits1; i++){
                  ...
                  const auto pct_i = (*pCalotime)[rh_i];
                  const auto pctof_i = (*pcTOF)[rh_i];
     	            auto Mini_t_i = t_i;
                  auto RtStc_t_i = 0.0;
     	     ...
     	     //std::cout << "Getting maps " << std::endl;
                  ...
                  sumXtalpCaloRecTime[id_i] += pct_t_i;
                  sumXtalRtStcRecTime[id_i] += RtStc_t_i;
                  sumXtalRtOOTStcRecTime[id_i] += RtOOTStc_t_i;
                  sumXtalWtStcRecTime[id_i] += WtStc_t_i;
                  sumXtalWtOOTStcRecTime[id_i] += WtOOTStc_t_i;
                  sumXtalMfOOTStcRecTime[id_i] += MfOOTStc_t_i;
                  sumXtalMfOOTCCStcRecTime[id_i] += MfOOTCCStc_t_i;
 ...
 //   } // end loop over inputfiles
    ...
    std::map<UInt_t,Float_t> *  icmaps[nAlgos] = {&sumXtalRtStcRecTime, &sumXtalRtOOTStcRecTime, &sumXtalWtStcRecTime, &sumXtalWtOOTStcRecTime, &sumXtalMfOOTStcRecTime, &sumXtalMfOOTCCStcRecTime, &sumXtalpCaloRecTime };
...
//                 std::cout << "Fill EM hist for Algo " << ai << " at " << fill_idinfo.i2 << " " << fill_idinfo.i1 << " with " << map_time << " for iter " << iter << std::endl;
    ...
    for( auto ai = 0; ai < nAlgos; ai++ ){ (*icmaps[ai]).clear();}
