...
void plot( string califilename, string infilename, string outfilename, string tvarname, string calimapname, string isd_type ){
...
//     get maps from fCaliFile
    ...
    for (auto entry = 0U; entry < nEntries; entry++){
        ...
        //std::cout << "Fill 2D Hist" << std::endl;
		  auto skip = false;
        auto kscc_offset = 0.f; //3101.f;
		  auto outlier = 1000.f;
		  auto offset = 0.f;
        auto outlier_offset = 0.f;
	     //if( tvarname == "phoseedkuMfootCCStctime" ) { outlier_offset = kscc_offset; } //std::cout << "KsCC ";} 
        if( (abs(phoseedtime_0 + outlier_offset ) > outlier) or (abs(phoseedtime_1 + outlier_offset ) > outlier) ) skip = true;
...
int main ( int argc, char *argv[] ){
        ...
        if( argc != 7 ) { std::cout << "Insufficent arguments." << std::endl; }
                ...
                auto isd_type = argv[6];
      			 plot( califilename, infilename, outfilename, tvarname, calimapname, isd_type );
        }
        return 1;
}
