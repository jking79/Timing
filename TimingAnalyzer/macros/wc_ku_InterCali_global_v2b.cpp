...
//			auto tmpsum1 = RtStc_t_i - prev_i[0] - RtStc_t_j + prev_j[0];
    ...
    //            >>>>  do subsums
		  ...
		  nM[iter] += eipow;
                  //std::cout << "M["<< iter << "] = " << M[iter] << " for " << nM[iter] << std::endl;
		  //normRtStc += subsum;
                  //normRtOOTStc += subsumnot;
                  //normWtOOTStc += subsumwoot;
                  sumXtalRtStcPhoIcRecTime[id_i] += eipow*(tot + prev_i[0]); //(subsum/subsumnum);
                  sumXtalRtOOTStcPhoIcRecTime[id_i] += eipow*(totnot + prev_i[1]); //(subsumnot/subsumnum);
