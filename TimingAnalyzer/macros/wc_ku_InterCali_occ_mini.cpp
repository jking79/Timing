...
//{sumXtalRtStcRecTime, sumXtalRtOOTStcRecTime }; //, sumXtalWtOOTStcRecTime };
	...
	string hnameEB( "Occ"+algostring[i]+"RecTimeEBMap"); 
        string htitleEB( "Occupancy "+algostring[i]+" RecTimeEBMap EB ");
        IcMapEB[i] = new TH2F(hnameEB.c_str(),htitleEB.c_str(),171,-85.5,85.5,360,0.5,360.5);
        IcMapEB[i]->Sumw2();
        string hnameEP( "Occ"+algostring[i]+"RecTimeEPMap"); 
        string htitleEP( "Occupancy "+algostring[i]+" RecTimeEPMap EB ");
        IcMapEP[i] = new TH2F(hnameEP.c_str(),htitleEP.c_str(),100,0.5,100.5,100,0.5,100.5);
        IcMapEP[i]->Sumw2();
        string hnameEM( "Occ"+algostring[i]+"RecTimeEMMap"); 
        string htitleEM( "Occupancy "+algostring[i]+" RecTimeEBMap EM ");
    ...
    for( auto ai = 0; ai < nAlgos; ai++ ){
	 //double drift = 0.0;
      //for( std::map<UInt_t,Float_t>::iterator it=(*icmaps[ai]).begin(); it!=(*icmaps[ai]).end(); ++it){ drift += (((*icmaps[ai])[it->first])/(numXtalIcRecTime[it->first])); }
         for( std::map<UInt_t,Float_t>::iterator it=numXtalIcRecTime.begin(); it!=numXtalIcRecTime.end(); ++it){
                   const auto & fill_idinfo = DetIDMap[it->first];
						 const auto & map_time = numXtalIcRecTime[it->first];
