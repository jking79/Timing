...
//     get maps from fCaliFile
    ...
    for (auto entry = 0U; entry < nEntries; entry++){
	 ...
	 if( entry%100000 == 0 ) std::cout << "Proccessed " << entry << " of " << nEntries << " entries." << std::endl;        
        ...
        auto eta_cut = ((phoseedEcal_0 == ECAL::EP)&&(phoseedEcal_1 == ECAL::EP))||((phoseedEcal_0 == ECAL::EM)&&(phoseedEcal_1 == ECAL::EM));
		  //auto bar_cut = (phoseedI2_0>0)&&( phoseedI2_1>0);
	     ...
	     auto event_good = e_cut && eta_cut && isd_cut; //&& bar_cut;

	     if(event_good) theHist->Fill(xfill,yfill);
