...
void wc_ku_rhCollectionPlots( std::string indir, std::string infilelist, std::string outfilename, std::string califilename ){

    const int  nAlgos = 3; // { RtStc, RtOOTStc, WtStc, WtOOTStc, kuMfootStc, kuMfootCCStc } -or- { Rt, Mini, WtStc, WtOOTStc }
    const int nOotAs = 10;
...
//    std::string algostring[nAlgos] = { "RtStc", "RtOOTStc", "WtStc", "WtNotStc", "kuMfootStc", "kuMfootCCStc" };
    std::string algostring[nAlgos] = { "Mini","kuMfootStc", "kuMfootCCStc" };
//    std::string calistring[nAlgos] = { "AveXtalMiniRecTime", "AveXtalMiniRecTime", "AveXtalRtStcRecTime", "AveXtalRtOOTStcRecTime", "AveXtalWtStcRecTime", 
//														"AveXtalWtOOTStcRecTime", "AveXtalMfootStcRecTime", "AveXtalMfootCCStcRecTime" };
//    std::string calistring[nAlgos] = { "AveXtalRtStcRecTime", "AveXtalRtOOTStcRecTime", "AveXtalWtStcRecTime",
//                                          "AveXtalWtOOTStcRecTime", "AveXtalMfootStcRecTime", "AveXtalMfootCCStcRecTime" };
    std::string calistring[nAlgos] = { "Mini","AveXtalMfootStcRecTime", "AveXtalMfootCCStcRecTime" };
...
//    std::string algostring[nAlgos] = { "Mini", "Rt", "WtStc", "WtNotStc" };
    ...
    for( auto i = 1; i < nAlgos; i++){
         ...
         ebmapic[i] = (TH2F*)fCaliFile->Get(ebmapstring.c_str());
         epmapic[i] = (TH2F*)fCaliFile->Get(epmapstring.c_str());
         emmapic[i] = (TH2F*)fCaliFile->Get(emmapstring.c_str());
  
		   std::cout << "Setting up Histograms for " << algostring[i] << " With Cali Maps: " << ebmapic[i] << " " << epmapic[i] << " " << emmapic[i] << std::endl;
         ...
         timeDist[i]->Sumw2();
		   ...
		   if( i < 0 ){ 
               ...
               std::string hnametd( "rht"+algostring[i]+"vMfoot");
               std::string htitletd( "rhTime "+algostring[i]+" vs Mfoot ");
...
//        std::vector<Float_t> * tvects[nAlgos] = { kuStcrhtime, kuNotStcrhtime, kuWtStcrhtime, kuWootStcrhtime, kuMfootStcrhtime, kuMfootCCStcrhtime };
        std::vector<Float_t> * tvects[nAlgos] = { rhtime, kuMfootStcrhtime, kuMfootCCStcrhtime };
                    ...
                    if( (*kurhID)[seed] == id_j ){
                            for(auto i = 1U; i < nAlgos; i++){ 
                                      ...
                                      if ( idinfo.ecal == ECAL::EB ){
                                               cali = ebmapic[i]->GetBinContent( idinfo.i2 + bin_offset, idinfo.i1 ) - adjust;
                                      }else if ( idinfo.ecal == ECAL::EP ){
                                               cali = epmapic[i]->GetBinContent( idinfo.i2, idinfo.i1 ) - adjust;
                                      }else if ( idinfo.ecal == ECAL::EM ){
                                               cali = emmapic[i]->GetBinContent( idinfo.i2, idinfo.i1 ) - adjust;
                                      }

											     times[i] = (*(tvects[i]))[seed] - cali; 
									 }
                            break;
                    }
            }
...
//				for(auto a = 0U; a < nOotAs; a++){ rhtvOotA[a][i]->Fill(times[i],oota[a]); }
            ...
            rhtvOotA9[i]->Fill(times[i],oota[9]);
				...
				if( i > 0 )  timeDistMaps[i]->Fill(times[i],times[0] );
			}
...
int main ( int argc, char *argv[] ){
	...
	if( argc != 5 ) { std::cout << "Insufficent arguments." << std::endl; }
                ...
                auto califilename = argv[4];
		          wc_ku_rhCollectionPlots( indir, infilename, outfilename, califilename );
	}
	return 1;
}
