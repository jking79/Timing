...
//    auto fInTree = (TTree*)fInFile->Get(disphotreename.c_str());
         ...
         string algostring[1] = { "Mini" }; //"RtStc", "RtOOTStc", "WtStc", "WtOOTStc" };
    ...
    while (std::getline(infile,str)){
    	 ...
    	 fInTree->SetBranchAddress("phorecHits_0",&npho_recHits_0,&b_npho_recHits_0);
    	 fInTree->SetBranchAddress("phorecHits_1",&npho_recHits_1,&b_npho_recHits_1);
    	 fInTree->SetBranchAddress("phorecHits_2",&npho_recHits_2,&b_npho_recHits_2);
    	 fInTree->SetBranchAddress("phorecHits_3",&npho_recHits_3,&b_npho_recHits_3);
         ...
         for (auto entry = 0U; entry < nEntries; entry++){
             ...
             //std::cout << "GetEntries rh for photons Finished "<< std::endl;
	     ...
	     for (auto c = 0U; c < nPhotons; c++){
             const auto nRecHits1 = (cluster[c])->size();
             //std::cout << "Looping over first recHits"  << std::endl;
             for (auto i = 0U; i < nRecHits1; i++){
                  const auto rh_i = (*(cluster[c]))[i]; // position within event rec hits vector
	     ...
	     } // end of loop over photon clusters
    ...
    std::map<UInt_t,Float_t> *  icmaps[nAlgos] = {&sumXtalMiniRecTime};//{&sumXtalRtStcRecTime, &sumXtalRtOOTStcRecTime }; //, sumXtalWtOOTStcRecTime };
	 ...
	 //double drift = 0.0;
      //for( std::map<UInt_t,Float_t>::iterator it=(*icmaps[ai]).begin(); it!=(*icmaps[ai]).end(); ++it){ drift += (((*icmaps[ai])[it->first])/(numXtalIcRecTime[it->first])); }
         for( std::map<UInt_t,Float_t>::iterator it=(*icmaps[ai]).begin(); it!=(*icmaps[ai]).end(); ++it){
                   ...
                   const auto & map_time = (((*icmaps[ai])[it->first])/(numXtalIcRecTime[it->first])) + offset; // - (drift/(icmaps[ai]->size()))) + offset;
