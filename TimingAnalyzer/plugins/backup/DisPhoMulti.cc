#include "Timing/TimingAnalyzer/plugins/DisPhoMulti.hh"
using namespace std;

DisPhoMulti::DisPhoMulti(const edm::ParameterSet & iConfig) :
...
//  lhcInfoValid (iConfig.existsAs<bool>("lhcInfoValid")  ? iConfig.getParameter<bool>("lhcInfoValid")  : false),
  ...
  kuMfootCCStcRecHitsEBTag(iConfig.getParameter<edm::InputTag>("kuMfootCCStcRecHitsEB")),
  kuMfootCCStcRecHitsEETag(iConfig.getParameter<edm::InputTag>("kuMfootCCStcRecHitsEE")),
...
void DisPhoMulti::ConsumeTokens()
{
  // triggers
  ...
  if( kuRechitValid ){
  	...
  	kuRecHitsEEToken = consumes<edm::SortedCollection<EcalRecHit,edm::StrictWeakOrdering<EcalRecHit> > > (kuRecHitsEETag);
        ...
        kuMfootCCStcRecHitsEBToken = consumes<edm::SortedCollection<EcalRecHit,edm::StrictWeakOrdering<EcalRecHit> > > (kuMfootCCStcRecHitsEBTag);
        kuMfootCCStcRecHitsEEToken = consumes<edm::SortedCollection<EcalRecHit,edm::StrictWeakOrdering<EcalRecHit> > > (kuMfootCCStcRecHitsEETag);
...
DisPhoMulti::~DisPhoMulti() {}

void DisPhoMulti::analyze(const edm::Event & iEvent, const edm::EventSetup & iSetup)
{
  ...
  //DisPhoMulti::GetLHCInfo(iEvent,iSetup);

  /////////////////
  // Get Weights //
  /////////////////
  //std::cout << ">>> Get Weights" << std::endl;
  DisPhoMulti::GetWeights();

  ///////////////////////
  // Always fill hists //
  ///////////////////////
  //std::cout << ">>> Always fill hists" << std::endl;
  DisPhoMulti::AlwaysFillHists();

  ////////////////////
  // Apply Blinding //
  ////////////////////
  //std::cout << ">>> Apply Blinding" << std::endl;
  //if (applyBlindSF && DisPhoMulti::ApplyBlindSF()) return;
  //DisPhoMulti::FillBlindSF();
  ...
  //if (applyBlindMET && DisPhoMulti::ApplyBlindMET()) return;
  //DisPhoMulti::FillBlindMET();
  ...
  //if (applyTrigger && DisPhoMulti::ApplyPreSelectionTrigger()) return;
  //DisPhoMulti::FillPreSelectionTrigger();

  if (applyHT && DisPhoMulti::ApplyPreSelectionHT()) return;
  DisPhoMulti::FillPreSelectionHT();

  //if (applyPhGood && DisPhoMulti::ApplyPreSelectionGoodPhoton()) return;

  DisPhoMulti::FillPreSelectionGoodPhoton();
...
bool DisPhoMulti::GetObjects(const edm::Event & iEvent, const edm::EventSetup & iSetup)
{
  //////////////////////
  // Standard Objects //
  //////////////////////

  if (!DisPhoMulti::GetStandardObjects(iEvent)) return false;

  ///////////////////////////
  // Calibration Constants //
  ///////////////////////////
  
  if (!DisPhoMulti::GetCalibrationConstants(iSetup)) return false;
  ...
  //if (isMC)
  //{
  //  if (!DisPhoMulti::GetMCObjects(iEvent)) return false;
  //}
...
//bool DisPhoMulti::GetLHCInfo(const edm::Event & iEvent, const edm::EventSetup & iSetup)
//{
//
...
bool DisPhoMulti::GetStandardObjects(const edm::Event & iEvent)
{
  // TRIGGER RESULTS
  ...
  if( kuRechitValid ){
  	...
  	if (oot::BadHandle(kuRecHitsEEH,"kuRecHitsEE")) return false;
        ...
        iEvent.getByToken(kuWtStcRecHitsEBToken,kuWtStcRecHitsEBH);
        if (oot::BadHandle(kuWtStcRecHitsEBH,"kuWtStcRecHitsEB")) return false;
        ...
        iEvent.getByToken(kuMfootStcRecHitsEEToken,kuMfootStcRecHitsEEH);
        if (oot::BadHandle(kuMfootStcRecHitsEEH,"kuMfootStcRecHitsEE")) return false;

        //iEvent.getByToken(kuMfootCCStcRecHitsEBToken,kuMfootCCStcRecHitsEBH);
        //if (oot::BadHandle(kuMfootCCStcRecHitsEBH,"kuMfootCCStcRecHitsEB")) return false;

        //iEvent.getByToken(kuMfootCCStcRecHitsEEToken,kuMfootCCStcRecHitsEEH);
        //if (oot::BadHandle(kuMfootCCStcRecHitsEEH,"kuMfootCCStcRecHitsEE")) return false;
...
bool DisPhoMulti::GetCalibrationConstants(const edm::EventSetup & iSetup)
{
...
bool DisPhoMulti::GetMCObjects(const edm::Event & iEvent)
{
  //////////////
  // GEN INFO //
  //////////////
...
void DisPhoMulti::InitializeObjects(const edm::Event & iEvent)
{
  ...
  if( kuRechitValid ){
  	...
  	kuRecHitsEE = kuRecHitsEEH.product();
        ...
        //kuMfootCCStcRecHitsEB = kuMfootCCStcRecHitsEBH.product();
        //kuMfootCCStcRecHitsEE = kuMfootCCStcRecHitsEEH.product();
  }
...
void DisPhoMulti::GetWeights()
{
  ...
  {  
    ...
    DisPhoMulti::InitializeGenPUBranches();
    DisPhoMulti::SetGenPUBranches();
  } // end check on isMC
...
void DisPhoMulti::InitializeGenPUBranches()
{
  genpuobs = -9999; 
  genputrue = -9999;
}

void DisPhoMulti::SetGenPUBranches()
{
...
void DisPhoMulti::AlwaysFillHists()
{
...
void DisPhoMulti::PrepObjects(const edm::Event & iEvent)
{
...
void DisPhoMulti::FillPreSelectionGoodPhoton()
{
...
void DisPhoMulti::FillTreeFromObjects(const edm::Event & iEvent)
{
  ...
  //if (isMC) DisPhoMulti::SetMCInfo();

  ///////////////////////////
  // Event, lumi, run info //
  ///////////////////////////
  //std::cout << ">>> Event, lumi, run info" << std::endl;
  DisPhoMulti::SetRecordInfo(iEvent);

  //////////////////
  // Trigger Info //
  //////////////////
  //std::cout << ">>> Trigger Info" << std::endl;
  DisPhoMulti::SetTriggerBranches();

  /////////////////////
  // MET Filter Info //
  /////////////////////
  //std::cout << ">>> MET Filter Info" << std::endl;
  DisPhoMulti::SetMETFilterBranches();

  /////////////////////////
  // Primary Vertex info //
  /////////////////////////
  //std::cout << ">>> Primary Vertex info" << std::endl;
  DisPhoMulti::InitializePVBranches();

  DisPhoMulti::SetPVBranches();

  //////////////////
  // Type1 PF Met //
  //////////////////
  //std::cout << ">>> Type1 PF Met" << std::endl;
  DisPhoMulti::InitializeMETBranches();
  DisPhoMulti::SetMETBranches();

  //if (isMC) 
  //{
  //  DisPhoMulti::InitializeMETBranchesMC();
  //  DisPhoMulti::SetMETBranchesMC();
  //}
  ...
  //DisPhoMulti::InitializeJetBranches();
  //DisPhoMulti::SetJetBranches();

  //if (isMC)
  //{
  //  DisPhoMulti::InitializeJetBranchesMC();
  //  DisPhoMulti::SetJetBranchesMC();
  //}
  ...
  DisPhoMulti::InitializeElectronBranches();
  DisPhoMulti::SetElectronBranches();

  ///////////
  // Muons //
  ///////////
  //std::cout << ">>> Muons" << std::endl;
  DisPhoMulti::InitializeMuonBranches();
  DisPhoMulti::SetMuonBranches();
  ...
  if( rawCollectionsValid ){
  	DisPhoMulti::InitializeDigiBranches();
	DisPhoMulti::SetDigiBranches();
  }
 ...
 // //std::cout << ">>> Reco Photons" << std::endl;
  DisPhoMulti::InitializePhoBranches();
  DisPhoMulti::SetPhoBranches();
  //if (isMC) 
  //{
  //  DisPhoMulti::InitializePhoBranchesMC();
  //  DisPhoMulti::SetPhoBranchesMC();
  //}
...
void DisPhoMulti::SetMCInfo()
{
  ////////////////
  // xyzt0 info //
  ////////////////
  
  DisPhoMulti::InitializeGenPointBranches();
  DisPhoMulti::SetGenT0Branches();
  DisPhoMulti::SetGenXYZ0Branches();
  ...
  if (isGMSB) 
  {
    DisPhoMulti::InitializeGMSBBranches();
    DisPhoMulti::SetGMSBBranches();
  } // isGMSB

  if (isHVDS) 
  {
    DisPhoMulti::InitializeHVDSBranches();
    DisPhoMulti::SetHVDSBranches();
  } // isHVDS

  if (isToy) 
  {
    DisPhoMulti::InitializeToyBranches();
    DisPhoMulti::SetToyBranches();
  } // isHVDS
}

void DisPhoMulti::InitializeGenPointBranches()
{
  gent0 = -9999.f;
  genx0 = -9999.f;
  geny0 = -9999.f;
...
void DisPhoMulti::SetGenT0Branches()
{
  gent0 = *(gent0H.product());
}

void DisPhoMulti::SetGenXYZ0Branches()
{
...
void DisPhoMulti::InitializeGMSBBranches()
{
  nNeutoPhGr = -9999;
...
void DisPhoMulti::SetGMSBBranches()
{
...
void DisPhoMulti::InitializeHVDSBranches()
{
  nvPions = -9999;
...
void DisPhoMulti::SetHVDSBranches()
{
  nvPions = vPions.size();
...
void DisPhoMulti::InitializeToyBranches()
{
  nToyPhs = -9999;
...
void DisPhoMulti::SetToyBranches()
{
  nToyPhs = toys.size();
...
void DisPhoMulti::SetRecordInfo(const edm::Event & iEvent)
{
  run   = iEvent.id().run();
...
void DisPhoMulti::SetTriggerBranches()
{
...
void DisPhoMulti::SetMETFilterBranches()
{
...
void DisPhoMulti::InitializePVBranches()
{
  nvtx = -9999; 
  vtxX = -9999.f;
  vtxY = -9999.f;
...
void DisPhoMulti::SetPVBranches()
{
...
void DisPhoMulti::InitializeMETBranches()
{
  t1pfMETpt    = -9999.f;
  t1pfMETphi   = -9999.f;
  t1pfMETsumEt = -9999.f;
...
void DisPhoMulti::SetMETBranches()
{
...
void DisPhoMulti::InitializeMETBranchesMC()
{
  genMETpt    = -9999.f;
  genMETphi   = -9999.f;
}

void DisPhoMulti::SetMETBranchesMC()
{
...
void DisPhoMulti::InitializeJetBranches()
{
  jetE.clear();
  jetpt.clear();
  jetphi.clear();
...
void DisPhoMulti::SetJetBranches()
{
  njets = jets.size();
...
void DisPhoMulti::InitializeJetBranchesMC()
{
  jetscaleRel.clear();
  jetsmearSF.clear();
...
void DisPhoMulti::SetJetBranchesMC()
{
  ...
  for (auto ijet = 0; ijet < nJets; ijet++)
  {
    ...
    // get genjet
    auto igenjet = DisPhoMulti::GenJetMatcher(jet,genJetsH,jer);
    ...
    {
      DisPhoMulti::GetStochasticSmear(mt_rand,jer,jer_sf     ,jetsmearSF    [ijet]); // nominal
      DisPhoMulti::GetStochasticSmear(mt_rand,jer,jer_sf_down,jetsmearDownSF[ijet]); // down
      DisPhoMulti::GetStochasticSmear(mt_rand,jer,jer_sf_up  ,jetsmearUpSF  [ijet]); // up
    ...
    DisPhoMulti::CheckJetSmear(energy,jetsmearSF    [ijet]);
    DisPhoMulti::CheckJetSmear(energy,jetsmearDownSF[ijet]);
    DisPhoMulti::CheckJetSmear(energy,jetsmearUpSF  [ijet]);
  }
}

int DisPhoMulti::GenJetMatcher(const pat::Jet & jet, const edm::Handle<std::vector<reco::GenJet> > & genJetsH, const float jer)
{
...
void DisPhoMulti::GetStochasticSmear(std::mt19937 & mt_rand, const float jer, const float jer_sf, float & jet_smear)
{
  if (jer_sf > 1.f)
  {
...
void DisPhoMulti::CheckJetSmear(const float energy, float & jet_smear)
{    
  if (energy * jet_smear < smearjetEmin) jet_smear = smearjetEmin / energy;
}

void DisPhoMulti::InitializeElectronBranches()
{
  nelLowL  = 0;
  nelLowM  = 0;
  nelLowT  = 0;
...
void DisPhoMulti::SetElectronBranches()
{
...
void DisPhoMulti::InitializeMuonBranches()
{
  nmuLowL  = 0;
  nmuLowM  = 0;
  nmuLowT  = 0;
...
void DisPhoMulti::SetMuonBranches()
{
...
void DisPhoMulti::InitializeRecHitBranches()
{
  rhX.clear();
  rhY.clear();
  rhZ.clear();
...
//  kuNotrhpedrms1.clear();
  ...
  kuWtStcrhE.clear();
  kuWtStcrhtime.clear();
  kuWtStcrhtimeErr.clear();
  kuWtStcrhTOF.clear();
  kuWtStcrhID.clear();
  kuWtStcrhisOOT.clear();
  kuWtStcrhisGS6.clear();
  kuWtStcrhisGS1.clear();
  kuWtStcrhadcToGeV.clear();
  kuWtStcrhped12.clear();
  kuWtStcrhped6.clear();
  kuWtStcrhped1.clear();
  kuWtStcrhpedrms12.clear();
  kuWtStcrhpedrms6.clear();
  kuWtStcrhpedrms1.clear();

  kuWootStcrhX.clear();
  kuWootStcrhY.clear();
  kuWootStcrhZ.clear();
  kuWootStcrhE.clear();
  kuWootStcrhtime.clear();
  kuWootStcrhtimeErr.clear();
  kuWootStcrhTOF.clear();
  kuWootStcrhID.clear();
  kuWootStcrhisOOT.clear();
  kuWootStcrhisGS6.clear();
  kuWootStcrhisGS1.clear();
  kuWootStcrhadcToGeV.clear();
  kuWootStcrhped12.clear();
  kuWootStcrhped6.clear();
  kuWootStcrhped1.clear();
  kuWootStcrhpedrms12.clear();
  kuWootStcrhpedrms6.clear();
  kuWootStcrhpedrms1.clear();

  kuMfootStcrhX.clear();
  kuMfootStcrhY.clear();
  kuMfootStcrhZ.clear();
  kuMfootStcrhE.clear();
  kuMfootStcrhtime.clear();
  kuMfootStcrhtimeErr.clear();
  kuMfootStcrhTOF.clear();
  kuMfootStcrhID.clear();
  kuMfootStcrhisOOT.clear();
  kuMfootStcrhisGS6.clear();
  kuMfootStcrhisGS1.clear();
  kuMfootStcrhadcToGeV.clear();
  kuMfootStcrhped12.clear();
  kuMfootStcrhped6.clear();
  kuMfootStcrhped1.clear();
  kuMfootStcrhpedrms12.clear();
  kuMfootStcrhpedrms6.clear();
  kuMfootStcrhpedrms1.clear();

  kuMfootCCStcrhX.clear();
  kuMfootCCStcrhY.clear();
  kuMfootCCStcrhZ.clear();
  kuMfootCCStcrhE.clear();
  kuMfootCCStcrhtime.clear();
  kuMfootCCStcrhtimeErr.clear();
  kuMfootCCStcrhTOF.clear();
  kuMfootCCStcrhID.clear();
  kuMfootCCStcrhisOOT.clear();
  kuMfootCCStcrhisGS6.clear();
  kuMfootCCStcrhisGS1.clear();
  kuMfootCCStcrhadcToGeV.clear();
  kuMfootCCStcrhped12.clear();
  kuMfootCCStcrhped6.clear();
  kuMfootCCStcrhped1.clear();
  kuMfootCCStcrhpedrms12.clear();
  kuMfootCCStcrhpedrms6.clear();
  kuMfootCCStcrhpedrms1.clear();
...
//  kuNotrhpedrms1.resize(nKuNotRecHits);
  ...
  kuWtStcrhX.resize(nKuWtStcRecHits);
  kuWtStcrhY.resize(nKuWtStcRecHits);
  kuWtStcrhZ.resize(nKuWtStcRecHits);
  kuWtStcrhE.resize(nKuWtStcRecHits);
  kuWtStcrhtime.resize(nKuWtStcRecHits);
  kuWtStcrhtimeErr.resize(nKuWtStcRecHits);
  kuWtStcrhTOF.resize(nKuWtStcRecHits);
  kuWtStcrhID.resize(nKuWtStcRecHits);
  kuWtStcrhisOOT.resize(nKuWtStcRecHits);
  kuWtStcrhisGS6.resize(nKuWtStcRecHits);
  kuWtStcrhisGS1.resize(nKuWtStcRecHits);
  kuWtStcrhadcToGeV.resize(nKuWtStcRecHits);
  kuWtStcrhped12.resize(nKuWtStcRecHits);
  kuWtStcrhped6.resize(nKuWtStcRecHits);
  kuWtStcrhped1.resize(nKuWtStcRecHits);
  kuWtStcrhpedrms12.resize(nKuWtStcRecHits);
  kuWtStcrhpedrms6.resize(nKuWtStcRecHits);
  kuWtStcrhpedrms1.resize(nKuWtStcRecHits);

  kuWootStcrhX.resize(nKuWootStcRecHits);
  kuWootStcrhY.resize(nKuWootStcRecHits);
  kuWootStcrhZ.resize(nKuWootStcRecHits);
  kuWootStcrhE.resize(nKuWootStcRecHits);
  kuWootStcrhtime.resize(nKuWootStcRecHits);
  kuWootStcrhtimeErr.resize(nKuWootStcRecHits);
  kuWootStcrhTOF.resize(nKuWootStcRecHits);
  kuWootStcrhID.resize(nKuWootStcRecHits);
  kuWootStcrhisOOT.resize(nKuWootStcRecHits);
  kuWootStcrhisGS6.resize(nKuWootStcRecHits);
  kuWootStcrhisGS1.resize(nKuWootStcRecHits);
  kuWootStcrhadcToGeV.resize(nKuWootStcRecHits);
  kuWootStcrhped12.resize(nKuWootStcRecHits);
  kuWootStcrhped6.resize(nKuWootStcRecHits);
  kuWootStcrhped1.resize(nKuWootStcRecHits);
  kuWootStcrhpedrms12.resize(nKuWootStcRecHits);
  kuWootStcrhpedrms6.resize(nKuWootStcRecHits);
  kuWootStcrhpedrms1.resize(nKuWootStcRecHits);

  kuMfootStcrhX.resize(nKuMfootStcRecHits);
  kuMfootStcrhY.resize(nKuMfootStcRecHits);
  kuMfootStcrhZ.resize(nKuMfootStcRecHits);
  kuMfootStcrhE.resize(nKuMfootStcRecHits);
  kuMfootStcrhtime.resize(nKuMfootStcRecHits);
  kuMfootStcrhtimeErr.resize(nKuMfootStcRecHits);
  kuMfootStcrhTOF.resize(nKuMfootStcRecHits);
  kuMfootStcrhID.resize(nKuMfootStcRecHits);
  kuMfootStcrhisOOT.resize(nKuMfootStcRecHits);
  kuMfootStcrhisGS6.resize(nKuMfootStcRecHits);
  kuMfootStcrhisGS1.resize(nKuMfootStcRecHits);
  kuMfootStcrhadcToGeV.resize(nKuMfootStcRecHits);
  kuMfootStcrhped12.resize(nKuMfootStcRecHits);
  kuMfootStcrhped6.resize(nKuMfootStcRecHits);
  kuMfootStcrhped1.resize(nKuMfootStcRecHits);
  kuMfootStcrhpedrms12.resize(nKuMfootStcRecHits);
  kuMfootStcrhpedrms6.resize(nKuMfootStcRecHits);
  kuMfootStcrhpedrms1.resize(nKuMfootStcRecHits);

//  kuMfootCCStcrhX.resize(nKuMfootCCStcRecHits);
//  kuMfootCCStcrhY.resize(nKuMfootCCStcRecHits);
//  kuMfootCCStcrhZ.resize(nKuMfootCCStcRecHits);
//  kuMfootCCStcrhE.resize(nKuMfootCCStcRecHits);
//  kuMfootCCStcrhtime.resize(nKuMfootCCStcRecHits);
//  kuMfootCCStcrhtimeErr.resize(nKuMfootCCStcRecHits);
//  kuMfootCCStcrhTOF.resize(nKuMfootCCStcRecHits);
//  kuMfootCCStcrhID.resize(nKuMfootCCStcRecHits);
//  kuMfootCCStcrhisOOT.resize(nKuMfootCCStcRecHits);
//  kuMfootCCStcrhisGS6.resize(nKuMfootCCStcRecHits);
//  kuMfootCCStcrhisGS1.resize(nKuMfootCCStcRecHits);
//  kuMfootCCStcrhadcToGeV.resize(nKuMfootCCStcRecHits);
//  kuMfootCCStcrhped12.resize(nKuMfootCCStcRecHits);
//  kuMfootCCStcrhped6.resize(nKuMfootCCStcRecHits);
//  kuMfootCCStcrhped1.resize(nKuMfootCCStcRecHits);
//  kuMfootCCStcrhpedrms12.resize(nKuMfootCCStcRecHits);
//  kuMfootCCStcrhpedrms6.resize(nKuMfootCCStcRecHits);
//  kuMfootCCStcrhpedrms1.resize(nKuMfootCCStcRecHits);
...
//  }
  ...
  for (auto i = 0; i < nKuWtStcRecHits; i++)
  {
    kuWtStcrhX[i] = -9999.f;
    kuWtStcrhY[i] = -9999.f;
    kuWtStcrhZ[i] = -9999.f;
    ...
    kuWtStcrhtime   [i] = -9999.f;
    kuWtStcrhtimeErr[i] = -9999.f;
    kuWtStcrhTOF    [i] = -9999.f;

    kuWtStcrhID[i] = 0; // non-ideal

    kuWtStcrhisOOT[i] = false;
    kuWtStcrhisGS6[i] = false;
    kuWtStcrhisGS1[i] = false;

    kuWtStcrhadcToGeV[i] = -9999.f;

    kuWtStcrhped12[i] = -9999.f;
    kuWtStcrhped6 [i] = -9999.f;
    kuWtStcrhped1 [i] = -9999.f;

    kuWtStcrhpedrms12[i] = -9999.f;
    kuWtStcrhpedrms6 [i] = -9999.f;
    kuWtStcrhpedrms1 [i] = -9999.f;
   }

  for (auto i = 0; i < nKuWootStcRecHits; i++)
  {
    kuWootStcrhX[i] = -9999.f;
    kuWootStcrhY[i] = -9999.f;
    kuWootStcrhZ[i] = -9999.f;
    kuWootStcrhE[i] = -9999.f;

    kuWootStcrhtime   [i] = -9999.f;
    kuWootStcrhtimeErr[i] = -9999.f;
    kuWootStcrhTOF    [i] = -9999.f;

    kuWootStcrhID[i] = 0; // non-ideal

    kuWootStcrhisOOT[i] = false;
    kuWootStcrhisGS6[i] = false;
    kuWootStcrhisGS1[i] = false;

    kuWootStcrhadcToGeV[i] = -9999.f;

    kuWootStcrhped12[i] = -9999.f;
    kuWootStcrhped6 [i] = -9999.f;
    kuWootStcrhped1 [i] = -9999.f;

    kuWootStcrhpedrms12[i] = -9999.f;
    kuWootStcrhpedrms6 [i] = -9999.f;
    kuWootStcrhpedrms1 [i] = -9999.f;
   }

  for (auto i = 0; i < nKuMfootStcRecHits; i++)
  {
    kuMfootStcrhX[i] = -9999.f;
    kuMfootStcrhY[i] = -9999.f;
    kuMfootStcrhZ[i] = -9999.f;
    kuMfootStcrhE[i] = -9999.f;

    kuMfootStcrhtime   [i] = -9999.f;
    kuMfootStcrhtimeErr[i] = -9999.f;
    kuMfootStcrhTOF    [i] = -9999.f;

    kuMfootStcrhID[i] = 0; // non-ideal

    kuMfootStcrhisOOT[i] = false;
    kuMfootStcrhisGS6[i] = false;
    kuMfootStcrhisGS1[i] = false;

    kuMfootStcrhadcToGeV[i] = -9999.f;

    kuMfootStcrhped12[i] = -9999.f;
    kuMfootStcrhped6 [i] = -9999.f;
    kuMfootStcrhped1 [i] = -9999.f;

    kuMfootStcrhpedrms12[i] = -9999.f;
    kuMfootStcrhpedrms6 [i] = -9999.f;
    kuMfootStcrhpedrms1 [i] = -9999.f;
   }

//  for (auto i = 0; i < nKuMfootCCStcRecHits; i++)
//  { 
//    kuMfootCCStcrhX[i] = -9999.f;
//    kuMfootCCStcrhY[i] = -9999.f;
//    kuMfootCCStcrhZ[i] = -9999.f;
//    kuMfootCCStcrhE[i] = -9999.f;
//    
//    kuMfootCCStcrhtime   [i] = -9999.f;
//    kuMfootCCStcrhtimeErr[i] = -9999.f;
//    kuMfootCCStcrhTOF    [i] = -9999.f;
//    
//    kuMfootCCStcrhID[i] = 0; // non-ideal
//    
//    kuMfootCCStcrhisOOT[i] = false;
//    kuMfootCCStcrhisGS6[i] = false;
//    kuMfootCCStcrhisGS1[i] = false;
//    
//    kuMfootCCStcrhadcToGeV[i] = -9999.f;
//    
//    kuMfootCCStcrhped12[i] = -9999.f;
//    kuMfootCCStcrhped6 [i] = -9999.f;
//    kuMfootCCStcrhped1 [i] = -9999.f;
//    
//    kuMfootCCStcrhpedrms12[i] = -9999.f;
//    kuMfootCCStcrhpedrms6 [i] = -9999.f;
//    kuMfootCCStcrhpedrms1 [i] = -9999.f;
//   }
...
void DisPhoMulti::InitializeDigiBranches()
{
...
void DisPhoMulti::SetRecHitBranches()
{
  nrechits = recHitMap.size();

  DisPhoMulti::SetRecHitBranches(recHitsEB,barrelGeometry,adcToGeVEB);
  DisPhoMulti::SetRecHitBranches(recHitsEE,endcapGeometry,adcToGeVEE);
  ...
  if( kuRechitValid ){	 
	...
	//for (const auto recHit : *kuRecHitsEB){//std::cout << "   Leading Pt KURecHit Time: " << recHit.time() << std::endl; break; }
  	...
  	DisPhoMulti::SetKuRecHitBranches(kuRecHitsEE,endcapGeometry,adcToGeVEE);
        ...
        //DisPhoMulti::SetKuMfootCCStcRecHitBranches(kuMfootCCStcRecHitsEB,barrelGeometry,adcToGeVEB);
        //DisPhoMulti::SetKuMfootCCStcRecHitBranches(kuMfootCCStcRecHitsEE,endcapGeometry,adcToGeVEE);
...
void DisPhoMulti::SetDigiBranches()
{
  ndigis = digiMap.size();
...
void DisPhoMulti::SetKuRecHitBranches(const EcalRecHitCollection * recHits, const CaloSubdetectorGeometry * geometry, const float adcToGeV)
{
...
void DisPhoMulti::SetKuStcRecHitBranches(const EcalRecHitCollection * recHits, const CaloSubdetectorGeometry * geometry, const float adcToGeV)
{
  //std::cout << ">>>>>>>> Setting KU Rechits" << std::endl;
  for (const auto recHit : *recHits)
  {
...
//void DisPhoMulti::SetKuNotRecHitBranches(const EcalRecHitCollection * recHits, const CaloSubdetectorGeometry * geometry, const float adcToGeV)
//{
//  //std::cout << ">>>>>>>> Setting KU Rechits" << std::endl;
//  for (const auto recHit : *recHits)
//  {
...
void DisPhoMulti::SetKuNotStcRecHitBranches(const EcalRecHitCollection * recHits, const CaloSubdetectorGeometry * geometry, const float adcToGeV)
{
  //std::cout << ">>>>>>>> Setting KU Rechits" << std::endl;
  for (const auto recHit : *recHits)
  {
...
void DisPhoMulti::SetKuWtStcRecHitBranches(const EcalRecHitCollection * recHits, const CaloSubdetectorGeometry * geometry, const float adcToGeV)
{
  //std::cout << ">>>>>>>> Setting KU Rechits" << std::endl;
  for (const auto recHit : *recHits)
  {
    const auto recHitId(recHit.detid());
    const auto rawId = recHitId.rawId();
    if (kuWtStcRecHitMap.count(rawId))
    {
      const auto pos = kuWtStcRecHitMap.at(rawId);
      const auto recHitPos = geometry->getGeometry(recHitId)->getPosition();

      // save position, energy, and time of each rechit to a vector
      kuWtStcrhX[pos] = recHitPos.x();
      kuWtStcrhY[pos] = recHitPos.y();
      kuWtStcrhZ[pos] = recHitPos.z();
      kuWtStcrhE[pos] = recHit.energy();

      // time info: compute TOF
      const auto d_orig = Config::hypo(kuWtStcrhX[pos],kuWtStcrhY[pos],kuWtStcrhZ[pos]);
      const auto d_pv   = Config::hypo(kuWtStcrhX[pos]-vtxX,kuWtStcrhY[pos]-vtxY,kuWtStcrhZ[pos]-vtxZ);
      kuWtStcrhtime   [pos] = recHit.time();
      kuWtStcrhtimeErr[pos] = recHit.timeError();
      kuWtStcrhTOF    [pos] = (d_orig-d_pv) / Config::sol;


      // detid
      kuWtStcrhID[pos] = rawId;

      // flags: isOOT, isGainSwitch6/1
      kuWtStcrhisOOT[pos] = recHit.checkFlag(EcalRecHit::kOutOfTime);
      kuWtStcrhisGS6[pos] = recHit.checkFlag(EcalRecHit::kHasSwitchToGain6);
      kuWtStcrhisGS1[pos] = recHit.checkFlag(EcalRecHit::kHasSwitchToGain1);

      // adcToGeVInfo : http://cmslxr.fnal.gov/source/RecoEcal/EgammaCoreTools/src/EcalClusterLazyTools.cc#0204
      const auto laser = laserH->getLaserCorrection(recHitId,evTime);
      const auto interCalibIter = interCalibMap->find(recHitId);
      const auto interCalib = ((interCalibIter != interCalibMap->end()) ? (*interCalibIter) : - 1.f);
      if ((laser > 0.f) && (interCalib > 0.f) && (adcToGeV > 0.f)) kuWtStcrhadcToGeV[pos] = (laser*interCalib*adcToGeV);
      ...
      if (pediter != pedestalsH->end())
      {
        ...
        kuWtStcrhped12[pos] = ped.mean(1);
        kuWtStcrhped6 [pos] = ped.mean(2);
        kuWtStcrhped1 [pos] = ped.mean(3);

        kuWtStcrhpedrms12[pos] = ped.rms(1);
        kuWtStcrhpedrms6 [pos] = ped.rms(2);
        kuWtStcrhpedrms1 [pos] = ped.rms(3);
      }
    }
  }
}

void DisPhoMulti::SetKuWootStcRecHitBranches(const EcalRecHitCollection * recHits, const CaloSubdetectorGeometry * geometry, const float adcToGeV)
{
  //std::cout << ">>>>>>>> Setting KU Rechits" << std::endl;
  for (const auto recHit : *recHits)
  {
    const auto recHitId(recHit.detid());
    const auto rawId = recHitId.rawId();
    if (kuWootStcRecHitMap.count(rawId))
    {
      const auto pos = kuWootStcRecHitMap.at(rawId);
      const auto recHitPos = geometry->getGeometry(recHitId)->getPosition();

      // save position, energy, and time of each rechit to a vector
      kuWootStcrhX[pos] = recHitPos.x();
      kuWootStcrhY[pos] = recHitPos.y();
      kuWootStcrhZ[pos] = recHitPos.z();
      kuWootStcrhE[pos] = recHit.energy();

      // time info: compute TOF
      const auto d_orig = Config::hypo(kuWootStcrhX[pos],kuWootStcrhY[pos],kuWootStcrhZ[pos]);
      const auto d_pv   = Config::hypo(kuWootStcrhX[pos]-vtxX,kuWootStcrhY[pos]-vtxY,kuWootStcrhZ[pos]-vtxZ);
      kuWootStcrhtime   [pos] = recHit.time();
      kuWootStcrhtimeErr[pos] = recHit.timeError();
      kuWootStcrhTOF    [pos] = (d_orig-d_pv) / Config::sol;


      // detid
      kuWootStcrhID[pos] = rawId;

      // flags: isOOT, isGainSwitch6/1
      kuWootStcrhisOOT[pos] = recHit.checkFlag(EcalRecHit::kOutOfTime);
      kuWootStcrhisGS6[pos] = recHit.checkFlag(EcalRecHit::kHasSwitchToGain6);
      kuWootStcrhisGS1[pos] = recHit.checkFlag(EcalRecHit::kHasSwitchToGain1);

      // adcToGeVInfo : http://cmslxr.fnal.gov/source/RecoEcal/EgammaCoreTools/src/EcalClusterLazyTools.cc#0204
      const auto laser = laserH->getLaserCorrection(recHitId,evTime);
      const auto interCalibIter = interCalibMap->find(recHitId);
      const auto interCalib = ((interCalibIter != interCalibMap->end()) ? (*interCalibIter) : - 1.f);
      if ((laser > 0.f) && (interCalib > 0.f) && (adcToGeV > 0.f)) kuWootStcrhadcToGeV[pos] = (laser*interCalib*adcToGeV);
      ...
      if (pediter != pedestalsH->end())
      {
        ...
        kuWootStcrhped12[pos] = ped.mean(1);
        kuWootStcrhped6 [pos] = ped.mean(2);
        kuWootStcrhped1 [pos] = ped.mean(3);

        kuWootStcrhpedrms12[pos] = ped.rms(1);
        kuWootStcrhpedrms6 [pos] = ped.rms(2);
        kuWootStcrhpedrms1 [pos] = ped.rms(3);
      }
    }
  }
}

void DisPhoMulti::SetKuMfootStcRecHitBranches(const EcalRecHitCollection * recHits, const CaloSubdetectorGeometry * geometry, const float adcToGeV)
{
  //std::cout << ">>>>>>>> Setting KU Rechits" << std::endl;
  for (const auto recHit : *recHits)
  {
    const auto recHitId(recHit.detid());
    const auto rawId = recHitId.rawId();
    if (kuMfootStcRecHitMap.count(rawId))
    {
      const auto pos = kuMfootStcRecHitMap.at(rawId);
      const auto recHitPos = geometry->getGeometry(recHitId)->getPosition();

      // save position, energy, and time of each rechit to a vector
      kuMfootStcrhX[pos] = recHitPos.x();
      kuMfootStcrhY[pos] = recHitPos.y();
      kuMfootStcrhZ[pos] = recHitPos.z();
      kuMfootStcrhE[pos] = recHit.energy();

      // time info: compute TOF
      const auto d_orig = Config::hypo(kuMfootStcrhX[pos],kuMfootStcrhY[pos],kuMfootStcrhZ[pos]);
      const auto d_pv   = Config::hypo(kuMfootStcrhX[pos]-vtxX,kuMfootStcrhY[pos]-vtxY,kuMfootStcrhZ[pos]-vtxZ);
      kuMfootStcrhtime   [pos] = recHit.time();
      kuMfootStcrhtimeErr[pos] = recHit.timeError();
      kuMfootStcrhTOF    [pos] = (d_orig-d_pv) / Config::sol;

      // detid
      kuMfootStcrhID[pos] = rawId;

      // flags: isOOT, isGainSwitch6/1
      kuMfootStcrhisOOT[pos] = recHit.checkFlag(EcalRecHit::kOutOfTime);
      kuMfootStcrhisGS6[pos] = recHit.checkFlag(EcalRecHit::kHasSwitchToGain6);
      kuMfootStcrhisGS1[pos] = recHit.checkFlag(EcalRecHit::kHasSwitchToGain1);

      // adcToGeVInfo : http://cmslxr.fnal.gov/source/RecoEcal/EgammaCoreTools/src/EcalClusterLazyTools.cc#0204
      const auto laser = laserH->getLaserCorrection(recHitId,evTime);
      const auto interCalibIter = interCalibMap->find(recHitId);
      const auto interCalib = ((interCalibIter != interCalibMap->end()) ? (*interCalibIter) : - 1.f);
      if ((laser > 0.f) && (interCalib > 0.f) && (adcToGeV > 0.f)) kuMfootStcrhadcToGeV[pos] = (laser*interCalib*adcToGeV);
      ...
      if (pediter != pedestalsH->end())
      {
        ...
        kuMfootStcrhped12[pos] = ped.mean(1);
        kuMfootStcrhped6 [pos] = ped.mean(2);
        kuMfootStcrhped1 [pos] = ped.mean(3);

        kuMfootStcrhpedrms12[pos] = ped.rms(1);
        kuMfootStcrhpedrms6 [pos] = ped.rms(2);
        kuMfootStcrhpedrms1 [pos] = ped.rms(3);
      }
    }
  }
}

void DisPhoMulti::SetKuMfootCCStcRecHitBranches(const EcalRecHitCollection * recHits, const CaloSubdetectorGeometry * geometry, const float adcToGeV)
{
  //std::cout << ">>>>>>>> Setting KU Rechits" << std::endl;
  for (const auto recHit : *recHits)
  {
    const auto recHitId(recHit.detid());
    const auto rawId = recHitId.rawId();
    if (kuMfootCCStcRecHitMap.count(rawId))
    {
      const auto pos = kuMfootCCStcRecHitMap.at(rawId);
      const auto recHitPos = geometry->getGeometry(recHitId)->getPosition();

      // save position, energy, and time of each rechit to a vector
      kuMfootCCStcrhX[pos] = recHitPos.x();
      kuMfootCCStcrhY[pos] = recHitPos.y();
      kuMfootCCStcrhZ[pos] = recHitPos.z();
      kuMfootCCStcrhE[pos] = recHit.energy();

      // time info: compute TOF
      const auto d_orig = Config::hypo(kuMfootCCStcrhX[pos],kuMfootCCStcrhY[pos],kuMfootCCStcrhZ[pos]);
      const auto d_pv   = Config::hypo(kuMfootCCStcrhX[pos]-vtxX,kuMfootCCStcrhY[pos]-vtxY,kuMfootCCStcrhZ[pos]-vtxZ);
      kuMfootCCStcrhtime   [pos] = recHit.time();
      kuMfootCCStcrhtimeErr[pos] = recHit.timeError();
      kuMfootCCStcrhTOF    [pos] = (d_orig-d_pv) / Config::sol;

      // detid
      kuMfootCCStcrhID[pos] = rawId;

      // flags: isOOT, isGainSwitch6/1
      kuMfootCCStcrhisOOT[pos] = recHit.checkFlag(EcalRecHit::kOutOfTime);
      kuMfootCCStcrhisGS6[pos] = recHit.checkFlag(EcalRecHit::kHasSwitchToGain6);
      kuMfootCCStcrhisGS1[pos] = recHit.checkFlag(EcalRecHit::kHasSwitchToGain1);

      // adcToGeVInfo : http://cmslxr.fnal.gov/source/RecoEcal/EgammaCoreTools/src/EcalClusterLazyTools.cc#0204
      const auto laser = laserH->getLaserCorrection(recHitId,evTime);
      const auto interCalibIter = interCalibMap->find(recHitId);
      const auto interCalib = ((interCalibIter != interCalibMap->end()) ? (*interCalibIter) : - 1.f);
      if ((laser > 0.f) && (interCalib > 0.f) && (adcToGeV > 0.f)) kuMfootCCStcrhadcToGeV[pos] = (laser*interCalib*adcToGeV);
      ...
      if (pediter != pedestalsH->end())
      {
        ...
        kuMfootCCStcrhped12[pos] = ped.mean(1);
        kuMfootCCStcrhped6 [pos] = ped.mean(2);
        kuMfootCCStcrhped1 [pos] = ped.mean(3);

        kuMfootCCStcrhpedrms12[pos] = ped.rms(1);
        kuMfootCCStcrhpedrms6 [pos] = ped.rms(2);
        kuMfootCCStcrhpedrms1 [pos] = ped.rms(3);
      }
    }
  }
}

void DisPhoMulti::SetURecHitBranches(const EcalUncalibratedRecHitCollection * recHits, const CaloSubdetectorGeometry * geometry)
{
  for (const auto recHit : *recHits)
    {
...
void DisPhoMulti::InitializePhoBranches()
{
...
void DisPhoMulti::SetPhoBranches()
{
...
void DisPhoMulti::InitializePhoBranchesMC()
{
  for (auto iphoton = 0; iphoton < Config::nPhotons; iphoton++)
  {
    auto & phoBranch = phoBranches[iphoton];
...
void DisPhoMulti::SetPhoBranchesMC()
{
  for (auto iphoton = 0; iphoton < nPhotons; iphoton++)
  {
    // get objects
    ...
    if (isHVDS) 
    {
      ...
      for (auto ihvds = 0; ihvds < Config::nHVDSs; ihvds++)
      {
	phoBranch.isSignal_ += (DisPhoMulti::CheckMatchHVDS(iphoton,hvdsBranches[ihvds]) * std::pow(10,ihvds));
...
int DisPhoMulti::CheckMatchHVDS(const int iphoton, const hvdsStruct & hvdsBranch)
{
...
void DisPhoMulti::MakeAndFillConfigTree()
{
...
void DisPhoMulti::MakeEventTree()
{
  // rho info
...
//    disphotree->Branch("kuNotStcrhpedrms1", &kuNotStcrhpedrms1);
    ...
    disphotree->Branch("kuWtStcrhtime", &kuWtStcrhtime);
    disphotree->Branch("kuWtStcrhtimeErr", &kuWtStcrhtimeErr);
    disphotree->Branch("kuWtStcrhID", &kuWtStcrhID);

//    disphotree->Branch("kuWootStcrhX", &kuWootStcrhX);
//    disphotree->Branch("kuWootStcrhY", &kuWootStcrhY);
//    disphotree->Branch("kuWootStcrhZ", &kuWootStcrhZ);
//    disphotree->Branch("kuWootStcrhE", &kuWootStcrhE);
    disphotree->Branch("kuWootStcrhtime", &kuWootStcrhtime);
    disphotree->Branch("kuWootStcrhtimeErr", &kuWootStcrhtimeErr);
//    disphotree->Branch("kuWootStcrhTOF", &kuWootStcrhTOF);
    disphotree->Branch("kuWootStcrhID", &kuWootStcrhID);
//    disphotree->Branch("kuWootStcrhisOOT", &kuWootStcrhisOOT);
//    disphotree->Branch("kuWootStcrhisGS6", &kuWootStcrhisGS6);
//    disphotree->Branch("kuWootStcrhisGS1", &kuWootStcrhisGS1);
//    disphotree->Branch("kuWootStcrhadcToGeV", &kuWootStcrhadcToGeV);
//    disphotree->Branch("kuWootStcrhped12", &kuWootStcrhped12);
//    disphotree->Branch("kuWootStcrhped6", &kuWootStcrhped6);
//    disphotree->Branch("kuWootStcrhped1", &kuWootStcrhped1);
//    disphotree->Branch("kuWootStcrhpedrms12", &kuWootStcrhpedrms12);
//    disphotree->Branch("kuWootStcrhpedrms6", &kuWootStcrhpedrms6);
//    disphotree->Branch("kuWootStcrhpedrms1", &kuWootStcrhpedrms1);

//    disphotree->Branch("kuMfootStcrhE", &kuMfootStcrhE);
    disphotree->Branch("kuMfootStcrhtime", &kuMfootStcrhtime);
    disphotree->Branch("kuMfootStcrhtimeErr", &kuMfootStcrhtimeErr);
//    disphotree->Branch("kuMfootStcrhTOF", &kuMfootStcrhTOF);
    disphotree->Branch("kuMfootStcrhID", &kuMfootStcrhID);

//    disphotree->Branch("kuMfootCCStcrhE", &kuMfootCCStcrhE);
//    disphotree->Branch("kuMfootCCStcrhtime", &kuMfootCCStcrhtime);
//    disphotree->Branch("kuMfootCCStcrhtimeErr", &kuMfootCCStcrhtimeErr);
//    disphotree->Branch("kuMfootCCStcrhTOF", &kuMfootCCStcrhTOF);
//    disphotree->Branch("kuMfootCCStcrhID", &kuMfootCCStcrhID);
...
void DisPhoMulti::endJob() {}

void DisPhoMulti::beginRun(edm::Run const & iRun, edm::EventSetup const & iSetup) {}

void DisPhoMulti::endRun(edm::Run const & iRun, edm::EventSetup const & iSetup) {}

void DisPhoMulti::fillDescriptions(edm::ConfigurationDescriptions & descriptions)
{
